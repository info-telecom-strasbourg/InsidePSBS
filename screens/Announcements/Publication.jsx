...
const Publication = ({data}) => {
    ...
    return (
        <TouchableOpacity activeOpacity={1} style={styles.container(theme)}
            ...
            <View style={{height: 5}}/>
            <Text style={text_styles.body3(theme)}>{hideTextOverflow(data.body, 500)}</Text>
            {data.body.length && (
                <TouchableOpacity style={{flexDirection: "row", alignItems: "flex-end", marginTop: 3}}
                                  onPress={() => router.push(`${ROUTES.publication}/${data.id}`)}>
                    <Text style={text_styles.body3({text: COLORS.primary})}>Voir plus</Text>
                    <View style={{width: 5}}/>
            ...
            <TouchableOpacity onPress={() => router.push(`${ROUTES.publication}/${data.id}`)}>
                <Text style={text_styles.body2({text: COLORS.primary})}>Aucun commentaire</Text>
            </TouchableOpacity>
        </TouchableOpacity>
    );
};
