      ...
      <>
        {res?.data.unistra_id != null ? (
          <Text style={text_styles.body1(theme)}>
            {"\n"}
            {TEXT.profile.unistra_user} :
            <Text style={{ fontWeight: "bold" }}>{res?.data.unistra_id}</Text>
          </Text>
        ) : (
          <Text style={text_styles.body1(theme)}>
            {"\n"}
            {TEXT.profile.unistra_no_user}
          </Text>
        )}
        <PrimaryButton
          style={{ marginTop: 50 }}
          text={TEXT.profile.unistra_connection}
          onPress={() => {
            setModalVisible(
              true,
            ); /* ; console.log("Pressed"); console.log(modalVisible) */
          }}
        />
        <Modal
          animationType="slide"
